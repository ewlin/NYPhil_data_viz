...
d3.json('/NYPhil_data_viz/data/new_top60.json', function (composers) {
	...
	var svg = d3.select('.main-container').append('svg').attr('width', SVG_WIDTH).attr('height', SVG_HEIGHT);
	/*
 ...
 */
	...
	var axisYears = d3.axisBottom(seasonsScale).tickValues(seasonsScale.domain().filter(function (season, i) {
		var S = ["1850-51", "1875-76", "1900-01", "1925-26", "1950-51", "1975-76", "2000-01", "2016-17"];
	...
	composers.forEach(function (composer, idx) {
		var option = '<option value=\'' + idx + '\'>' + composer.composer + '</option>';
	...
	function renderDots(number) {
		...
		ALL_SEASONS.forEach(function (season, season_idx) {
			...
			works.forEach(function (work, work_idx) {
				...
				if (workSeasons.includes(season)) {
					var workMetaData = { id: composerIndex + ':' + work_idx,
						title: work.title,
						season: season,
		...
		d3.select('.lifetime-box').select('line').transition().duration(1400).attr('x1', rectX).attr('x2', rectX + rectWidth);
		...
		var dots = svg.selectAll('circle').data(beethovenWorks);
	...
	var beethoven = composers[0];
	var composerIndex = 0;
	console.log(beethoven);

	var composer = beethoven.composer;
	var composerImage = composer.toLowerCase().split(' ')[0].match(/[a-z]*/)[0] + '.png';
	$('.composer-face-container').append('<img class=\'composer-face\' src=\'assets/images/composer_sqs/' + composerImage + '\'/>');

	var birthSeason = beethoven.birth + "-" + (+beethoven.birth.substr(2) + 1);
	console.log(birthSeason);
	var deathSeason = beethoven.death + "-" + (+beethoven.death.substr(2) + 1);
	console.log(deathSeason);
	var rectX = seasonsScale(birthSeason) ? seasonsScale(birthSeason) : seasonsScale("1842-43");
	var rectWidth = void 0;
	...
	//		let birthSeason = d.birth + "-" + (+d.birth.substr(2) + 1); 
	//		return seasonsScale(birthSeason) ? seasonsScale(birthSeason) : -100; 
	.attr('y', 0).attr('width', rectWidth)
	//		let birthSeason = d.birth + "-" + (+d.birth.substr(2) + 1); 
	//		let deathSeason = d.death + "-" + (+d.death.substr(2) + 1); 
	//		if (seasonsScale(birthSeason))
	.attr('height', SVG_HEIGHT * .92).attr('opacity', .3).attr('fill', 'grey');
	...
	svg.selectAll('.piece').data(beethovenWorks).enter().append('circle').attr('class', 'piece').attr('r', seasonsScale.bandwidth() / 2.4).attr('cx', function (d) {
		return seasonsScale(d.season);
	}).attr('cy', function (d) {
		return yScale(d.seasonWorkCount);
	}).attr('fill', function (d) {
